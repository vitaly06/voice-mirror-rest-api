...
try {
    ...
    Write-Host "❌ Docker не установлен или не запущен. Установите Docker Desktop для Windows." -ForegroundColor Red
    exit 1
}
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            try {
                ...
                if ($response.StatusCode -eq 200) {
                    ...
                    Write-Host "  Wav2Lip доступен: $($healthData.wav2lip_available)" -ForegroundColor White
                    Write-Host "  Модель доступна: $($healthData.model_available)" -ForegroundColor White
                    Write-Host "  FFmpeg доступен: $($healthData.ffmpeg_available)" -ForegroundColor White
                } else {
        ...
        Write-Host "Проверьте логи сборки выше" -ForegroundColor Yellow
        exit 1
    }
} catch {
